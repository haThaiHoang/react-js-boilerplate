...
import { CHECK_STATUS, CHECK_STATUS_OPTIONS } from '@/constants'
import { Table, PaginationBar, Button, Select, Column } from '@/components'
...
class ReconfirmFromInputCard extends Component {
  constructor(props) {
    super(props)
    ...
    this._columns = [{
      ...
      render: (cell, record) => (
        <Icon
          onClick={() => this._updateApplicationModal.open(record, 'reconfirm-input-card-information')}
  ...
  _getApplications = (newParams) => {
    ...
    let payload = {
      ...params,
      ...newParams,
      type: 'reconfirm-input-card-info'
    }
  ...
  render() {
    ...
    if (!permissions.includes('EDIT_APPLICATION_BY_STATUS_RE_CONFIRM_INPUT_CARD_INFORMATION')) {
      columns = columns.filter(item => item.key !== 'action')
    }

    if (!permissions.includes('UPDATE_STATUS_RE_CONFIRM_INPUT_CARD_INFORMATION_TO_PAID_CHECKED')) {
    ...
    return (
      <div className="paid-uncheck">
        <div className="top-action-box">
          ...
          {permissions.includes('UPDATE_STATUS_RE_CONFIRM_INPUT_CARD_INFORMATION_TO_PAID_CHECKED') && (
            <div className="button-group">
              <Checkbox
        ...
        <UpdateToPaidCheckModal
          ...
          type="reconfirm-input-card-information-to-paid-checked"
        />
...
export default ReconfirmFromInputCard
