...
class PaidChecked extends Component {
  constructor(props) {
    super(props)
    ...
    this._columns = [{
      ...
      render: (cell, record) => (
        <Icon
          onClick={() => this._updateApplicationModal.open(record, 'paid-checked')}
  ...
  render() {
    ...
    if (!permissions.includes('EDIT_APPLICATION_BY_STATUS_PAID_CHECKED')) {
      columns = columns.filter(item => item.key !== 'action')
    }

    if (!permissions.includes('EXPORT_CSV_BY_STATUS_PAID_CHECKED')) {
    ...
    return (
      <div className="paid-uncheck">
        <div className="top-action-box">
          ...
          {permissions.includes('EXPORT_CSV_BY_STATUS_PAID_CHECKED') && (
            <div className="button-group">
              <Checkbox
              ...
              <Button
                type="primary"
                size="small"
                onClick={() => { this._csvDownloadModal.open() }}
        ...
        />
        <CsvDownloadModal
          innerRef={(ref) => { this._csvDownloadModal = ref }}
