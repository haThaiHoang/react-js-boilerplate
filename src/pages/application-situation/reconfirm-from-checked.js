...
class ReconfirmFromChecked extends Component {
  constructor(props) {
    super(props)
    ...
    this._columns = [{
      ...
      render: (cell, record) => (
        <Icon
          onClick={() => this._updateApplicationModal.open(record, 'reconfirm-paid-checked')}
  ...
  render() {
    ...
    if (!permissions.includes('EDIT_APPLICATION_BY_STATUS_RE_CONFIRM_PAID_CHECKED')) {
      columns = columns.filter(item => item.key !== 'action')
    }

    if (!permissions.includes('UPDATE_STATUS_RE_CONFIRM_PAID_CHECKED_TO_PAID_CHECKED')) {
    ...
    return (
      <div className="paid-uncheck">
        <div className="top-action-box">
          ...
          {permissions.includes('UPDATE_STATUS_RE_CONFIRM_PAID_CHECKED_TO_PAID_CHECKED') && (
            <div className="button-group">
              <Checkbox
        ...
        <UpdateToPaidCheckModal
          ...
          type="reconfirm-paid-checked-to-paid-checked"
        />
...
export default ReconfirmFromChecked
